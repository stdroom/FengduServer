var YDataGrid = function(config){
		...
		var Action = {
			...
			'print': actionUrl.print||'print.do'
		}
		...
		var Handler = {
			...
			print:function(callback){
					$.messager.confirm('Confirm','确定要打印吗?',function(r){  
					    if (r){
					    YiYa.progress();
					    alert(1);
					    YiYa.printForm(Action.print,data,function(result){
						YiYa.closeProgress(); 						//回调函数
						if(jQuery.isFunction(callback)){
							callback(result);
								}
							});
			...
			save: function(callback){
				...
				var url="/ms/view/testprintPass.jsp?flag=1&hwmc="+hwmc+"&jianshu="+jianshu+"&shrlxdh="+shrlxdh
		        +"&bgf="+bgf+"&yf="+yf+"&zfy="+zfy+"&fkfs="+fkfs;
		...
		var initForm = function(){
			...
			if(Form.search && Form.search[0]){
				Form.search.find("#btn-print").click(Events.print); //打印事件
			}
		}
